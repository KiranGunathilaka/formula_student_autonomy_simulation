...
struct Landmark
{
  ...
  // to 1 so a color only wins if it has at least one vote. If no color has
  // any votes yet we fall back to COLOR_UNKNOWN, which is the correct semantic
  // for a landmark whose color is not yet determined.
  // ---------------------------------------------------------------------------
  uint8_t dominantColor() const
  {
    uint8_t  best       = COLOR_UNKNOWN;
    uint32_t best_count = 3;  // require at least 1 vote to win
